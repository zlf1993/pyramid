...
class CentLoss(tf.keras.layers.Layer):
    ...
    def _compute_one_image_loss(self, gravity_pred, dist_pred, heatmap_pred, ground_truth):
        ...
        iou_loss = tf.reduce_sum(-tf.math.log(iou + 1e-12) * ioc * (1.0 + iou_reduction))
        ...
        seg_pos_loss = - 500 * tf.pow(1. - tf.sigmoid(heatmap_pred), 2.) * tf.math.log_sigmoid(heatmap_pred) * gt_seg
        ...
        if tf.reduce_sum(gt_seg) != 0:
            ...
            iou_loss = iou_loss / tf.reduce_sum(ioc)
            ...
            return center_loss, iou_loss, size_loss, seg_loss, total_loss
        else:
            return .0, .0, .0, .0, .0
...
class CenterNet:
    ...
    def _build_backbone(self, x):
        ...
        # x = BottleNeck(in_size=80, exp_size=184, out_size=80, s=1, is_se_existing=False, NL="HS", k=3)(x)
        # x = BottleNeck(in_size=80, exp_size=184, out_size=80, s=1, is_se_existing=False, NL="HS", k=3)(x)
        # x = BottleNeck(in_size=80, exp_size=120, out_size=112, s=1, is_se_existing=False, NL="HS", k=3)(x)
        # s_8 = BottleNeck(in_size=112, exp_size=256, out_size=112, s=1, is_se_existing=False, NL="HS", k=3)(x)
    ...
    def _build_graph(self):
        ...
        keypoints, preg, fpn = self._fusion_feature(s_4, s_8, s_16, s_32)
    ...
    def _SepConv_BN(self, x, filters, prefix, stride=1, kernel_size=3, rate=1, depth_activation=False, point_bn=True, point_activation=False, epsilon=1e-3):
