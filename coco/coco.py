    ...
    def generator(self, image_id):
        ...
        if gt is None:
            ...
            for i in range(instance_num):
                ...
                vertical_pads = np.pad(vertical_ones, [[0, 0], [pad_l, pad_r]], "constant", constant_values=(int(gt_left[i][0]/self.config.STRIDE), int(gt_right[i][0]/self.config.STRIDE)))
                ...
                horizontal_pads = np.pad(horizontal_ones, [[pad_t, pad_b], [0, 0]], "constant", constant_values=(int(gt_top[i][0]/self.config.STRIDE), int(gt_bot[i][0]/self.config.STRIDE)))
            ...
            zero_like = np.zeros((int(self.config.IMAGE_MAX_DIM/self.config.STRIDE), int(self.config.IMAGE_MAX_DIM/self.config.STRIDE), 1), np.float32)
            gt_keypoints = []
            heatmap_gt = []
            reduction_gt = []
            for i in range(self.num_classes):
                ...
                if np.shape(reduce_i)[-1] == 0:
                    ...
                    gt_keypoints_i = csr_matrix((np.ones(num_i), (gy, gx)),
                                                shape=(int(self.config.IMAGE_MAX_DIM/self.config.STRIDE), int(self.config.IMAGE_MAX_DIM/self.config.STRIDE))).toarray()
